...
export default async function LatestActivitiesList({
  userId,
  friendId,
  friendUsername,
}: {
  ...
  let latestPayFriendTransactions: TransactionItem[] = [];
  const {
    status: payFriendTransactionsStatus,
    message: payFriendTransactionsMessage,
    data: payFriendTransactionsData,
  } = await getPayFriendTransactions(userId, friendId, 10);
  if (payFriendTransactionsStatus !== "success" || !payFriendTransactionsData) {
    console.error(payFriendTransactionsMessage || "Error fetching latest pay friend transactions");
  } else {
    latestPayFriendTransactions = payFriendTransactionsData["transactionItems"];
  }

  if (latestPayFriendTransactions.length === 0) {
    return (
      <div className="flex flex-col justify-center items-center w-full h-48 my-6 border-0 rounded-xl bg-gray-100">
        ...
        <p className="px-2 text-center text-sm">
          Start creating payments with {friendUsername} to view latest activities
        </p>
      </div>
    );
  }
  ...
  return (
    <ul className="flex flex-col items-center gap-3 w-full py-4 max-md:py-3">
      {latestPayFriendTransactions.map((transaction, idx) => {
        return (
          <li
                ...
                <p className={clsx(
                  ...
                  {formatCurrency(
                    Math.trunc(transaction.amount * (
                      transaction.recipient_id === userId && transaction.exchange_rate ? transaction.exchange_rate : 1
                    )),
